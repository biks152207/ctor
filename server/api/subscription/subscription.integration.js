...
describe('Subscription API:', function() {
  ...
  describe('PUT /api/subscriptions/:id', function() {
    ...
    it('should respond with the original subscription', function() {
      expect(updatedSubscription.name).to.equal('New Subscription');
      expect(updatedSubscription.info).to.equal('This is the brand new subscription!!!');
    });
  ...
  describe('PATCH /api/subscriptions/:id', function() {
    ...
    beforeEach(function(done) {
      request(app)
        .patch(`/api/subscriptions/${newSubscription._id}`)
        .send([
